...
while True:
    length = input("Please enter the maximum string length (the value must be greater than 35 characters):\n")
    if length.isdigit() == False:
        print("String length must be a number. \n")
        continue
    if int(length) < 35:
        print("String length must be greater than 35 characters. \n")
        continue
    break
...
with open("text.txt", 'r+', encoding='utf-8') as donor:
    with open("text_wrap.txt", 'w+', encoding='utf-8') as recipient:
        for line in donor:
            start_index = 0
            ...
            while finish_index <= last_space:
                if len(line) - finish_index > length:
                    ...
                    break
        print("DONE! The edited file is saved in text_wrap.txt")        