...
#        print('DB created')
        cur = conn.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS materials (id  integer not null primary key, type text, manufacturer text, trade_mark text, series text, color text, effects text, nominal_amount real, real_amount real, measure_units text, unit_price_BYN real, unit_price_USD real, purchase_date integer, purchase_place text, update_timestamp integer, comment text)")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS accessories (id integer not null primary key, type text, manufacturer text, description text, unit_price_BYN real, unit_price_USD real, purchase_date integer, purchase_place text, update_timestamp integer, comment text)")
        cur.execute(
