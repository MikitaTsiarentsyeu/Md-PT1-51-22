...
async def train_all(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.replay(f"{data['step']}. Das word ist {data ['word']}", reply_markup=inline_kb)
    ...
    async with state.proxy() as data:
        if answer == data.get(answer):
            ...
            if res:
                ...
                data['word'] = res.get('word')
                data['pk'] = res.get('pk')
                await bot.send_message(callback_query.from_user.id, f"{data['step']}. Das word ist {data ['word']}", reply_markup=inline_kb)
            else:
